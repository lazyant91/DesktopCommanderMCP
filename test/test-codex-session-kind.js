...
async function inspect(command, expected, shell) {
  let pid = -1;
  try {
    const result = await terminalManager.executeCommand(command, 500, shell);
    pid = result.pid;
...
async function run() {
  if (process.platform === 'win32') {
    ...
    await inspect('@ cmd.exe /d /k echo ready', 'shell', 'cmd.exe');
