...
The process tools do not receive trusted origin metadata, so the server applies this reminder to every matching Local MCP `start_process` call and recognized owned-shell `interact_with_process` input. A human who wants to use Codex directly must start it independently in a local terminal rather than through these Local MCP process tools.

The reminder covers direct `codex` launcher names, the official `@openai/codex` npm execution forms including arguments passed to that package, and the same commands sent to recognized owned interactive shells: cmd, PowerShell, pwsh, bash, sh, and zsh. A single leading CMD echo-control `@` is removed before executable-name matching and shell-session classification, whether it is attached to the executable token (`@codex`) or is the first standalone token (`@ codex`). The bounded npm forms are `npx` with optional `-y` or `--yes` followed by an optional `--`, and `npm exec` or `npm x` with an optional `--` before the package token. The package token may be exactly `@openai/codex` or the same package followed by a non-empty version or dist-tag specifier such as `@openai/codex@latest`; similarly named packages are not matched. It is independent of the editable `blockedCommands` list. Other AI tools, ordinary Git/npm/build/test commands, package metadata operations, paths containing the word `codex`, and strings in non-shell REPL sessions are not restricted by this feature.
...
For CMD, the first `/c` or `/k` token selects the host mode; later occurrences belong to the command text and do not change whether the owned session remains input-capable.

The bounded PowerShell/pwsh classifier recognizes `-ExecutionPolicy`, `-WorkingDirectory`, `-InputFormat`, and `-OutputFormat` as options that consume exactly one following value. `-Command` and `-File` are normally one-shot execution forms, but when their following value is exactly `-` they read commands or script text from stdin and are classified as input-capable shells. `-NoExit` keeps an execution session classified as a shell only when it appears before the execution target; a trailing `-NoExit` after `-Command`, `-File`, or a positional script is treated as target input rather than a host option. For bash, sh, and zsh, exact `-s` stdin mode remains `shell` even when later positional values are script arguments; `-c`, a script path before `-s`, and `-s` after `--` remain `other`. It does not infer option abbreviations or parse additional value-consuming options. POSIX shell value-consuming startup options, such as `bash --rcfile profile.sh -i`, remain outside this bounded classifier.

Shell-specific escaping and multiline syntax are not interpreted. Only double quotes are treated as cross-shell protection for a segment separator; single quotes are still accepted during tokenization but do not hide `;`, `|`, `&`, LF, or CRLF because CMD does not treat them as quoting. A backslash is also not treated as a cross-shell escape for a segment separator. PowerShell or POSIX single-quoted data, PowerShell backtick continuation, CMD caret continuation, POSIX backslash continuation, and POSIX heredoc content can therefore be conservatively refused when a later segment resembles a recognized launch. Use a separate operating-system account or virtual machine when stronger isolation is required.
